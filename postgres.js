...
DB.prototype.insert = function(table,values, id='id'){
	...
	return new Promise(function(resolve,reject){
		conn.make_client().then(function(client){
			...
			conn.query(client,sql+= 'ON CONFLICT DO NOTHING RETURNING ' + id, params, resolve);
		})
	})
}


//Bulk INSERT statement handler. The values parameter is an object whose property names represent the columns and the value in for each property should be an array
